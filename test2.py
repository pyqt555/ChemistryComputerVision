...
def count_objects(frame, prev_frame, count_cross, direction, middle_line):
...
while True:
    ...
    count_cross, direction = count_objects(b_arr, prev_frame, count_cross, direction, middle_line)

    # Visualize the direction with arrows
    if direction == 1:
        cv2.arrowedLine(b_arr, (middle_line, b_arr.shape[0] // 2), (b_arr.shape[1], b_arr.shape[0] // 2),
                        (0, 255, 0), 2, tipLength=0.05)
    elif direction == -1:
        cv2.arrowedLine(b_arr, (middle_line, b_arr.shape[0] // 2), (0, b_arr.shape[0] // 2),
                        (0, 255, 0), 2, tipLength=0.05)
...
print("Direction of objects:", direction)