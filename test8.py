...
def initialize_trackers(frame, prev_frame, object_trackers, tracker_type='CSRT'):
    frame_uint8 = frame.astype(np.uint8)
    prev_frame_uint8 = prev_frame.astype(np.uint8)

    # Compute absolute difference between frames
    frame_diff = np.abs(prev_frame_uint8 - frame_uint8)
    ...
    for contour in contours:
        ...
        new_obj_id = max([obj['id'] for obj in object_trackers], default=0) + 1
...
def count_objects(frame, middle_line, draw_frame, tracked_objects, counters, object_trackers, tracker_type='CSRT'):
    ...
    if len(tracked_objects) > 0:
        # Initialize trackers using contours only when objects are already being tracked
        initialize_trackers(frame, frame - tracked_objects[-1]['frame'], object_trackers, tracker_type)
